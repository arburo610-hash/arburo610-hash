...
def load_finished_posts():
    """Загружает утверждённые/опубликованные посты из готовые_посты.md"""
    try:
        with open("готовые_посты.md", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        return "Готовых постов пока нет."


CONTENT_PLAN = load_content_plan()
VOICE_EXAMPLES = load_voice_examples()
FINISHED_POSTS = load_finished_posts()
...
3. Сверяйся с КОНТЕНТ-ПЛАНОМ и списком УЖЕ ГОТОВЫХ постов — не предлагай темы, которые уже написаны, запланированы или опубликованы.
...
Не «исполнитель за м²» и не «мастер-одиночка, который всё делает руками сам», а специалист, который лично ведёт проект и держит на себе главные решения и ответственность за результат.

Важно про «всё сама»: позиционирование — про контроль и ответственность, а не про буквальное «делаю всё одна». Помощники и подрядчики на отдельных задачах — норма и не скрываются; со временем появится команда, но точка ответственности одна — это Анастасия. В текстах НЕ использовать «только я», «всё сама», «не передаю команде»; использовать «веду лично», «главные решения и ответственность на мне», «за проект целиком отвечает один человек».
...
## УЖЕ ГОТОВЫЕ / ОПУБЛИКОВАННЫЕ ПОСТЫ

Эти тексты уже утверждены или опубликованы. НЕ предлагай их заново и не дублируй закрытые здесь темы.
